  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className='w-full flex-col py-3 mb-4 '>
        ...
        />
        <input className="bg-black/60 border-[1px] border-gray-100/50 rounded-sm w-full p-4 text-white mb-8" type="password"
          placeholder='Password'
          autoComplete='current password'
          value={password}
          onChange={(e) => setPassword(e.target.value)}
        />
        <button onClick={() => handleClick(email, password)} className='btn-red px-4 py-3 w-full'>{title}</button>
      </form>
    </>
